  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (response.user && response.token) {
        setStoredAuthSession(response.user, response.token);
      }
        ...
        <div className="w-full max-w-md">
          ...
          {isGoogleSignInConfigured() && (
            <button
              ...
              className="w-full flex items-center justify-center gap-3 border border-[#E5E7EB] bg-white rounded-2xl py-3.5 mb-4 hover:bg-[#F9FAFB] transition-colors disabled:cursor-not-allowed disabled:opacity-70"
            >
              ...
              <span className="text-[#374151] text-sm" style={{ fontWeight: 500 }}>
                {isGoogleSubmitting ? "Connecting to Google..." : t.continueWithGoogle}
              </span>
            </button>
          )}
