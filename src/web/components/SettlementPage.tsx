...
import { getExpenses, settleExpense, type Expense } from "../api/expenses";
...
function getCurrentUserShare(expense: Expense, currentUserId: string | undefined) {
  return (
...
function getOthersShare(expense: Expense, currentUserId: string | undefined) {
  ...
  const relevantExpenses = useMemo(() => {
    return expenses.filter((expense) => {
      ...
      return (
        ...
        expense.participants.some((participant) => participant.userId === currentUser.id)
      );
    });
  ...
  const youAreOwedByCurrency = useMemo(() => {
    ...
    pendingExpenses.forEach((expense) => {
      ...
      upsertCurrencyTotal(
        totals,
        expense.currency,
        getOthersShare(expense, currentUser?.id),
      );
    });
  ...
  const youOweByCurrency = useMemo(() => {
    ...
    pendingExpenses.forEach((expense) => {
      ...
      upsertCurrencyTotal(
        totals,
        expense.currency,
        getCurrentUserShare(expense, currentUser?.id),
      );
    });
